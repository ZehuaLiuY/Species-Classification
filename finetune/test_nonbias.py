...
desired_names = set(name.lower() for name in Class_names.values())
...
parser.add_argument("--train_type", choices=['single', 'ddp'], default='single',
...
def collate_fn_remove_none(batch):
    ...
    """
    filtered_batch = [item for item in batch if item is not None and item[0] is not None]
...
def filter_vehicle(dataset):
    """
    filter out the vehicle class from the subset
    """
    filtered_indices = []
    if hasattr(dataset, "indices"):
        for idx in tqdm(range(len(dataset)), desc="Filtering vehicle"):
            item = dataset[idx]
            ...
            if target["labels"].numel() > 0:
                ...
                cname = dataset.dataset.categories.get(label, "").lower()
                if cname != "vehicle":
...
def test_model(model, loader, criterion, device, transform=None, num_class=None, class_names=None, remap=None, filter_vehicle_in_test=False):
    ...
    with torch.no_grad():
        for batch_idx, (images, targets) in enumerate(tqdm(loader, desc="Testing")):
            ...
            if remap is not None:
                predicted = torch.tensor([remap[p.item()] if p.item() in remap else p.item() for p in predicted], device=device)
            ...
            for idx in range(len(batch_results)):
                ...
                batch_results[idx]['predicted_class'] = class_names.get(pred_label, f"Unknown({pred_label})")
                batch_results[idx]['ground_truth_class'] = class_names.get(gt_label, f"Unknown({gt_label})")
    ...
    with open("../test_result/nonbiased/json/48/CE_Adam.json", "w", encoding="utf-8") as f:
...
def main(args):
    ...
    sample_category = []
    for i in tqdm(range(len(dataset)), desc="Counting class frequencies"):
        _, target = dataset[i]
        if target is None:
            continue
        if target["labels"].numel() > 0:
            label = target["labels"][0].item()
            cname = dataset.categories.get(label, "").lower()
            sample_category.append(cname)
            if cname in desired_names:
                freq_counter[cname] += 1
        else:
            sample_category.append("")
    print("Test set has :", dict(freq_counter))

    filtered_indices = []
    for i, cname in enumerate(sample_category):
        if cname in desired_names and freq_counter[cname] > 1:
    ...
    if num_class == 48:
        ...
        filtered_test_indices = filter_vehicle(filtered_dataset)
        print("finished filtering")
        filtered_dataset = Subset(filtered_dataset, filtered_test_indices)
    ...
    with open("../test_result/nonbiased/txt/48/CE_Adam.txt", "w", encoding="utf-8") as f:
        ...
        for i, label in enumerate(valid_labels):
            class_name = f"Class {i} ({display_class_names.get(label, 'Unknown')})"
            line = (
                f"{class_name:<35}"
